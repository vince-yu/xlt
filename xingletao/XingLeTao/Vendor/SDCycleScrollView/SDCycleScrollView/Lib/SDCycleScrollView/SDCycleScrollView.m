...
- (void)setPlaceholderImage:(UIImage *)placeholderImage
{
    ...
    if (!self.backgroundImageView) {
        UIImageView *letaobgImageView = [UIImageView new];
        letaobgImageView.contentMode = UIViewContentModeScaleAspectFit;
        [self insertSubview:letaobgImageView belowSubview:self.mainView];
        self.backgroundImageView = letaobgImageView;
    }
    // 
    self.backgroundImageView.backgroundColor = [UIColor letaolightgreyBgSkinColor];
...
@implementation XLTGoodDetailSDCycleScrollView
/* 裁剪底部轮播图底部代码
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    SDCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:ID forIndexPath:indexPath];
    ...
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            [self imageView:cell.imageView setImageWithURL:[NSURL URLWithString:imagePath] placeholderImage:self.placeholderImage];
        } else {
            UIImage *image = [UIImage imageNamed:imagePath];
            if (!image) {
                image = [UIImage imageWithContentsOfFile:imagePath];
...
- (void)imageView:(UIImageView *)imageView setImageWithURL:(nullable NSURL *)url placeholderImage:(nullable UIImage *)placeholder {
    ...
    [imageView sd_setImageWithURL:url placeholderImage:placeholder options:SDWebImageAvoidAutoSetImage completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        if (!error) {
            UIImage *resizeImage = [weakSelf resizeImage:image];
...
- (UIImage *)resizeImage:(UIImage *)image {
    if (image) {
        ...
        if (height > self.bounds.size.height) {
            // 裁剪图片
            CGRect clipRect = CGRectMake(0, 0, width, self.bounds.size.height);
...
@implementation XLTHomePageCycleScrollView 
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    [super scrollViewDidScroll:scrollView];
    
    CGFloat totalImagewidth = self.bounds.size.width * self.imagePathsGroup.count;
    CGFloat currentOffset = self.mainView.contentOffset.x - self.bounds.size.width * 50 * self.imagePathsGroup.count;
    NSInteger realOffset;
    if (currentOffset > 0) {
        realOffset = (NSInteger)currentOffset % (NSInteger)totalImagewidth;
    } else {
        realOffset = (NSInteger)currentOffset % (NSInteger)totalImagewidth + totalImagewidth;
    }
    
    if ([self.delegate respondsToSelector:@selector(cycleMainScrollViewDidScroll:offset:)]) {
        [self.mainScrollViewDelegate cycleMainScrollViewDidScroll:scrollView offset:(NSInteger)realOffset];
    }
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.onlyDisplayText && [imagePath isKindOfClass:[NSString class]]) {
        if ([imagePath hasPrefix:@"http"]) {
            NSString *cacheImageKey = [[SDWebImageManager sharedManager] cacheKeyForURL:[NSURL URLWithString:imagePath]];
