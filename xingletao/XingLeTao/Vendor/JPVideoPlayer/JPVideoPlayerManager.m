...
didReceiveLoadingRequestTask:(JPResourceLoadingRequestWebTask *)requestTask {
    JPVideoPlayerDownloaderOptions downloaderOptions = [self xingletaonetwork_requestDownloadOptionsWithOptions:videoPlayer.playerModel.playerOptions];
...
playerStatusDidChange:(JPVideoPlayerStatus)playerStatus {
    if(playerStatus == JPVideoPlayerStatusReadyToPlay){
        if([self xingletaonetwork_requestPlaybackRecordForVideoURL:self.managerModel.videoURL] > 0){
            BOOL shouldSeek = YES;
            if (self.delegate && [self.delegate respondsToSelector:@selector(videoPlayerManager:shouldResumePlaybackFromPlaybackRecordForURL:elapsedSeconds:)]) {
                shouldSeek = [self.delegate videoPlayerManager:self
                  shouldResumePlaybackFromPlaybackRecordForURL:self.managerModel.videoURL
                                                elapsedSeconds:[self xingletaonetwork_requestPlaybackRecordForVideoURL:self.managerModel.videoURL]];
            }
            if(shouldSeek){
                [self.videoPlayer seekToTimeWhenRecordPlayback:CMTimeMakeWithSeconds([self xingletaonetwork_requestPlaybackRecordForVideoURL:self.managerModel.videoURL], 1000)];
            }
        }
    }
...
- (long long)xingletaonetwork_requestFileSizeAtPath:(NSString *)filePath{
...
- (JPVideoPlayerDownloaderOptions)xingletaonetwork_requestDownloadOptionsWithOptions:(JPVideoPlayerOptions)options {
...
- (void)playLocalVideoWithShowLayer:(CALayer *)showLayer
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:path]) {
        ...
        self.managerModel.fileLength = (NSUInteger)[self xingletaonetwork_requestFileSizeAtPath:path];;
...
- (double)xingletaonetwork_requestPlaybackRecordForVideoURL:(NSURL *)videoURL {
    if(!videoURL){
