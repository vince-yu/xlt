        ...
        if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
            ...
            if (!self.selectionIndicatorStripLayer.superlayer) {
                ...
                if (self.selectionStyle == HMSegmentedControlSelectionStyleBox && !self.selectionIndicatorBoxLayer.superlayer) {
                    //change
                    if (self.useCustomIndicatorBox) {
                        // 修正量
                        CGFloat offset = 2;
                        CGRect fillerRect =[self frameForFillerSelectionIndicator];
                        self.selectionIndicatorBoxLayer.frame = CGRectMake(fillerRect.origin.x, fillerRect.origin.y +self.segmentEdgeInset.top -offset, fillerRect.size.width, fillerRect.size.height - self.segmentEdgeInset.top-self.segmentEdgeInset.bottom);
                        self.selectionIndicatorBoxLayer.cornerRadius = ceilf(self.selectionIndicatorBoxLayer.frame.size.height/2);
                    } else {
