...
#import "SDSecurityPolicy.h"
#import "SDConfigOptions.h"
#import "SDConstants.h"

NS_ASSUME_NONNULL_BEGIN

@class SndoDataPeople;

/**
 * @abstract
 * 在 DEBUG 模式下，发送错误时会抛出该异常
 */
@interface SndoDataDebugException : NSException
...
//UITableView
@optional
- (NSDictionary *)sndoAnalytics_tableView:(UITableView *)tableView autoTrackPropertiesAtIndexPath:(NSIndexPath *)indexPath;

//UICollectionView
@optional
- (NSDictionary *)sndoAnalytics_collectionView:(UICollectionView *)collectionView autoTrackPropertiesAtIndexPath:(NSIndexPath *)indexPath;
@end

@interface UIImage (SndoData)
@property (nonatomic, copy) NSString* sndoAnalyticsImageName;
@end

@interface UIView (SndoData)
- (nullable UIViewController *)sndoAnalyticsViewController __attribute__((deprecated("已过时")));

/// viewID
@property (nonatomic, copy) NSString* sndoAnalyticsViewID;

/// AutoTrack 时，是否忽略该 View
@property (nonatomic, assign) BOOL sndoAnalyticsIgnoreView;

/// AutoTrack 发生在 SendAction 之前还是之后，默认是 SendAction 之前
@property (nonatomic, assign) BOOL sndoAnalyticsAutoTrackAfterSendAction;

/// AutoTrack 时，View 的扩展属性
@property (nonatomic, strong) NSDictionary* sndoAnalyticsViewProperties;

@property (nonatomic, weak, nullable) id<SAUIViewAutoTrackDelegate> sndoAnalyticsDelegate;
@end
 ...
 *
 * @abstract
 * 在 SDK 中嵌入 SndoData 的 SDK 并进行使用的主要 API
 *
 * @discussion
 * 使用 SndoDataSDK 类来跟踪用户行为，并且把数据发给所指定的 SndoData 的服务。
 * 它也提供了一个 SndoDataPeople 类型的 property，用来访问用户 Profile 相关的 API。
 */
@interface SndoDataSDK : NSObject

/**
 * @property
 *
 * @abstract
 * 对 SndoDataPeople 这个 API 的访问接口
 */
@property (atomic, readonly, strong) SndoDataPeople *people;
...
/**
 ...
 默认使用 defaultPolicy
 */
@property (nonatomic, strong) SDSecurityPolicy *securityPolicy;
 ...
 */
+ (void)startWithConfigOptions:(nonnull SDConfigOptions *)configOptions NS_SWIFT_NAME(start(configOptions:));
 ...
 * @return 返回的单例
 */
+ (SndoDataSDK * _Nullable)sharedInstance;
 ...
 *
 */
@property (nonatomic, getter = getMaxCacheSize) UInt64 maxCacheSize  __attribute__((deprecated("已过时，请参考 SDConfigOptions 类的 maxCacheSize")));
- (UInt64)getMaxCacheSize __attribute__((deprecated("已过时，请参考 SDConfigOptions 类的 maxCacheSize")));
 ...
 *
 * @param networkType SndoDataNetworkType
 */
- (void)setFlushNetworkPolicy:(SndoDataNetworkType)networkType;
 ...
 */
- (void)trackAppCrash  __attribute__((deprecated("已过时，请参考 SDConfigOptions 类的 enableTrackAppCrash")));
 ...
 *   https://sndodata.cn/manual/ios_sdk.html
 * 该功能默认关闭
 */
- (void)enableAutoTrack:(SndoDataAutoTrackEventType)eventType __attribute__((deprecated("已过时，请参考 SDConfigOptions 类的 autoTrackEventType")));
 ...
 *
 * @param eventType SndoDataAutoTrackEventType 要判断的 AutoTrack 事件类型
 *
 * @return YES:被忽略; NO:没有被忽略
 */
- (BOOL)isAutoTrackEventTypeIgnored:(SndoDataAutoTrackEventType)eventType;
 ...
 *
 * 这个接口是一个较为复杂的功能，请在使用前先阅读相关说明: https://sndodata.cn/manual/track_installation.html，并在必要时联系我们的技术支持人员。
 *
 ...
 *
 * @discussion
 * 根据需要决定是否开启 SDK log , SndoDataDebugOff 模式默认关闭 log
 * SndoDataDebugOnly  SndoDataDebugAndTrack 模式默认开启log
 *
 ...
 *
 * @discussion
 * <b>请不要自己来初始化这个类.</b> 请通过 SndoDataSDK 提供的 people 这个 property 来调用
 */
@interface SndoDataPeople : NSObject
...
@interface SndoDataSDK (Deprecated)
 ...
 */
@property (atomic) UInt64 flushInterval __attribute__((deprecated("已过时，请参考 SDConfigOptions 类的 flushInterval")));
 ...
 */
@property (atomic) UInt64 flushBulkSize __attribute__((deprecated("已过时，请参考 SDConfigOptions 类的 flushBulkSize")));

/**
 * @abstract
 * 根据传入的配置，初始化并返回一个 SndoDataSDK 的单例
 *
 @param configOptions 参数配置
 @return 返回的单例
 */
+ (SndoDataSDK *)sharedInstanceWithConfig:(nonnull SDConfigOptions *)configOptions __attribute__((deprecated("已过时，请使用 + (void)startWithConfigOptions: 方法进行初始化")));
 ...
 * @return 返回的单例
 */
+ (SndoDataSDK *)sharedInstanceWithServerURL:(nullable NSString *)serverURL
                                        andDebugMode:(SndoDataDebugMode)debugMode __attribute__((deprecated("已过时，请参考 sharedInstanceWithConfig:")));
 ...
 * @return 返回的单例
 */
+ (SndoDataSDK *)sharedInstanceWithServerURL:(nonnull NSString *)serverURL
                                    andLaunchOptions:(NSDictionary * _Nullable)launchOptions
                                        andDebugMode:(SndoDataDebugMode)debugMode __attribute__((deprecated("已过时，请参考 sharedInstanceWithConfig:")));
/**
 * @abstract
 * 根据传入的配置，初始化并返回一个 SndoDataSDK 的单例。
 * 目前 DebugMode 为动态开启，详细请参考说明文档：https://www.sndodata.cn/manual/ios_sdk.html
 * @param serverURL 收集事件的 URL
 * @param launchOptions launchOptions
 *
 * @return 返回的单例
 ...
 */
- (void)setDebugMode:(SndoDataDebugMode)debugMode __attribute__((deprecated("已过时，建议动态开启调试模式")));
 ...
 * 该功能默认关闭
 */
- (void)enableAutoTrack __attribute__((deprecated("已过时，请参考 SDConfigOptions 类的 autoTrackEventType")));

/**
 * @abstract
 * 过滤掉 AutoTrack 的某个事件类型
 *
 * @param eventType SndoDataAutoTrackEventType 要忽略的 AutoTrack 事件类型
 */
- (void)ignoreAutoTrackEventType:(SndoDataAutoTrackEventType)eventType __attribute__((deprecated("已过时，请参考enableAutoTrack:(SndoDataAutoTrackEventType)eventType")));
 ...
 *
 * @discussion
 * 这个接口是一个较为复杂的功能，请在使用前先阅读相关说明: http://www.sndodata.cn/manual/track_signup.html，并在必要时联系我们的技术支持人员。
 *
 ...
 */
- (void)trackTimerBegin:(NSString *)event withTimeUnit:(SndoDataTimeUnit)timeUnit __attribute__((deprecated("已过时，请参考 trackTimerStart")));
 ...
 * @param timeUnit          计时单位，毫秒/秒/分钟/小时
 */
- (void)trackTimer:(NSString *)event withTimeUnit:(SndoDataTimeUnit)timeUnit __attribute__((deprecated("已过时，请参考 trackTimerStart")));
 ...
 */
- (void)enableVisualizedAutoTrack __attribute__((deprecated("已过时，请参考 SDConfigOptions 类的 enableVisualizedAutoTrack")));

/**
 开启 HeatMap，$AppClick 事件将会采集控件的 viewPath
 */
- (void)enableHeatMap __attribute__((deprecated("已过时，请参考 SDConfigOptions 类的 enableHeatMap")));
