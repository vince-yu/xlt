...
@implementation XLTNomalAlterView
- (instancetype)initWithNib {
    self = [[[NSBundle mainBundle] loadNibNamed:@"XLTNomalAlterView" owner:nil options:nil] lastObject];
    if (self) {
...
+ (void)showNamalAlterWithTitle:(NSString *)title content:(NSString *)content leftBlock:(void(^)(void))leftblock rightBlock:(void(^)(void))rightBlock{
    XLTNomalAlterView *alter = [[XLTNomalAlterView alloc] initWithNib];
...
+ (void)showNamalAlterWithTitle:(NSString *)title content:(NSString *)content leftStr:(NSString *)leftStr rightStr:(NSString *)rightStr leftBlock:(void(^)(void))leftblock rightBlock:(void(^)(void))rightBlock{
    XLTNomalAlterView *alter = [[XLTNomalAlterView alloc] initWithNib];
    alter.leftBlock = leftblock;
    alter.rightBlock = rightBlock;
    alter.title = title;
    alter.contentStr = content;
    if (leftStr) {
        [alter.leftBtn setTitle:leftStr forState:UIControlStateNormal];
    }
    if (rightStr) {
        [alter.rightBtn setTitle:rightStr forState:UIControlStateNormal];
    }
    [alter show];
}
