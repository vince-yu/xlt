...
@interface XLTHomeDiscoverGoodsCell ()
...
@implementation XLTHomeDiscoverGoodsCell
    ...
    if ([modulesItemArray isKindOfClass:[NSArray class]]) {
        [modulesItemArray enumerateObjectsUsingBlock:^(XLTHomeModuleItemModel *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([info isKindOfClass:[NSDictionary class]]) {
                XLTHomeDiscoverGoodsItemView *itemView = [self itemViewForIndex:idx];
                itemView.tag = idx;
                [itemView letaoUpdateInfo:info];
        ...
        if (modulesItemCount < self.itemArray.count) {
            ...
            [self.itemArray enumerateObjectsUsingBlock:^(XLTHomeDiscoverGoodsItemView *  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (XLTHomeDiscoverGoodsItemView *)itemViewForIndex:(NSUInteger)index {
    if (index < self.itemArray.count) {
        return self.itemArray[index];
    } else {
        XLTHomeDiscoverGoodsItemView *itemView = [[[NSBundle mainBundle] loadNibNamed:@"XLTHomeDiscoverGoodsItemView" owner:nil options:nil] lastObject];
        itemView.backgroundColor = [UIColor colorWithHex:0xFFFAFAFA];
...
- (void)addjustItemView:(XLTHomeDiscoverGoodsItemView *)itemView contentHeight:(CGFloat)contentHeight atIndex:(NSUInteger)index {
