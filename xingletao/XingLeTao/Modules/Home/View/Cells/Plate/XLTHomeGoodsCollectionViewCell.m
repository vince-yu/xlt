...
@interface XLTHomeGoodsCollectionViewCell ()
...
@property (nonatomic, weak) IBOutlet UIButton *letaoBuyFlagBtn;
@property (nonatomic, weak) IBOutlet UILabel *letaoGoodsImageFlagLabel;

@end
@implementation XLTHomeGoodsCollectionViewCell
...
- (void)awakeFromNib {
    ...
    self.letaoBuyFlagBtn.layer.masksToBounds = YES;
    self.letaoBuyFlagBtn.layer.cornerRadius = 5.0;
    UIImage *bgImage = [UIImage gradientColorImageFromColors:@[[UIColor colorWithHex:0xFFFF6E02],[UIColor colorWithHex:0xFFFFAE01]] gradientType:1 imgSize:CGSizeMake(25, 25)];
    [self.letaoBuyFlagBtn setBackgroundImage:bgImage forState:UIControlStateNormal];
...
- (void)letaoUpdateCellDataWithInfo:(id _Nullable )itemInfo {
    ...
    if (![paidNumber isKindOfClass:[NSNumber class]]
        ...
        self.salesAmountLabel.text = [NSString stringWithFormat:@"已抢%@件",@"0"];
    } else {
        self.salesAmountLabel.text = [NSString stringWithFormat:@"已抢%@件",[paidNumber letaoTenThousandNumberFormat]];
    }
