...
@implementation XLTStoreTopHeadView
...
- (void)awakeFromNib {
    ...
    self.letaodescribeLabel.adjustsFontSizeToFitWidth = YES;
    self.letaoserviceLabel.adjustsFontSizeToFitWidth = YES;
    self.letaologisticsLabel.adjustsFontSizeToFitWidth = YES;
}
...
- (void)letaoHiddenStoreInfo:(BOOL)hidden {
    ...
    [self.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (hidden) {
            obj.hidden = (obj != self.letaobgImageView);
        } else {
            obj.hidden = NO;
        }
    }];
...
- (void)letaoUpdateStoreWithDictionary :(NSDictionary * )letaoStoreDictionary {
    ...
    if (!([seller_evaluates isKindOfClass:[NSArray class]] && seller_evaluates.count > 0)) {
        seller_evaluates = [XLTStoreTopHeadView letaoStoreBuildSellerEvaluatesArrayWithInfo:letaoStoreDictionary];
    }
    self.letaodescribeLabel.attributedText = ([seller_evaluates isKindOfClass:[NSArray class]] && seller_evaluates.count >0) ? [XLTStoreTopHeadView letaoFormattingEvaluates:seller_evaluates[0]] : nil;
    self.letaoserviceLabel.attributedText = ([seller_evaluates isKindOfClass:[NSArray class]] && seller_evaluates.count >1) ? [XLTStoreTopHeadView letaoFormattingEvaluates:seller_evaluates[1]] : nil;
    self.letaologisticsLabel.attributedText = ([seller_evaluates isKindOfClass:[NSArray class]] && seller_evaluates.count >2) ? [XLTStoreTopHeadView letaoFormattingEvaluates:seller_evaluates[2]] : nil;
...
+ (NSAttributedString *)letaoFormattingEvaluates:(NSDictionary *)evaluates {
    ...
    [attributedString addAttributes:@{NSForegroundColorAttributeName:[XLTStoreTopHeadView letaoStoreLevelColorForType:level]} range:NSMakeRange(attributedString.length - score.length, score.length)];
...
+ (BOOL)evaluatesValid:(NSDictionary * )letaoStoreDictionary {
    if ([letaoStoreDictionary isKindOfClass:[NSDictionary class]]) {
        NSArray *seller_evaluates = letaoStoreDictionary[@"seller_evaluates"];
        if (!([seller_evaluates isKindOfClass:[NSArray class]] && seller_evaluates.count > 0)) {
            seller_evaluates = [XLTStoreTopHeadView letaoStoreBuildSellerEvaluatesArrayWithInfo:letaoStoreDictionary];
