...
@implementation XLTShareFeedTopCell
...
- (void)letaoUpdateCellDataWithInfo:(NSDictionary *)info {
    NSString *avatar = nil;
    NSString *recom_title = nil;
    NSNumber *start_time = nil;
    NSNumber *counts = nil;
    NSDictionary *goodBase = nil;
    if ([info isKindOfClass:[NSDictionary class]]) {
        if ([info[@"avatar"] isKindOfClass:[NSString class]]) {
            avatar = info[@"avatar"];
        }
        if ([info[@"recom_title"] isKindOfClass:[NSString class]]) {
            recom_title = info[@"recom_title"];
        }
    ...
    [self.iconImageView sd_setImageWithURL:[NSURL URLWithString:avatar]];
    self.nameLabel.text = recom_title;
    ...
    [self.letaoShareBtn setTitle:[NSString stringWithFormat:@" %ld",[counts longValue]] forState:UIControlStateNormal];
