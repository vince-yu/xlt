...
@interface XLTCelebrityStoreHeaderView ()
...
@implementation XLTCelebrityStoreHeaderView
...
- (void)awakeFromNib {
    ...
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(letaoGoStoreAction:)];
...
- (void)updateCelebrityData:(NSDictionary * )letaoStoreDictionary {
    ...
    if (!([seller_evaluates isKindOfClass:[NSArray class]] && seller_evaluates.count > 0)) {
        seller_evaluates = [XLTCelebrityStoreHeaderView letaoStoreBuildSellerEvaluatesArrayWithInfo:letaoStoreDictionary];
    }
    self.letaodescribeLabel.attributedText = ([seller_evaluates isKindOfClass:[NSArray class]] && seller_evaluates.count >0) ? [XLTCelebrityStoreHeaderView letaoFormattingEvaluates:seller_evaluates[0]] : nil;
    self.letaoserviceLabel.attributedText = ([seller_evaluates isKindOfClass:[NSArray class]] && seller_evaluates.count >1) ? [XLTCelebrityStoreHeaderView letaoFormattingEvaluates:seller_evaluates[1]] : nil;
    self.letaologisticsLabel.attributedText = ([seller_evaluates isKindOfClass:[NSArray class]] && seller_evaluates.count >2) ? [XLTCelebrityStoreHeaderView letaoFormattingEvaluates:seller_evaluates[2]] : nil;
    ...
    [self letaoUpdateJDdongWithCreditLevel:type jd_self:jd_self credit_level:credit_level];
...
+ (NSAttributedString *)letaoFormattingEvaluates:(NSDictionary *)evaluates {
    ...
    [attributedString addAttributes:@{NSForegroundColorAttributeName:[XLTCelebrityStoreHeaderView letaoStoreLevelColorForType:level]} range:NSMakeRange(attributedString.length - score.length, score.length)];
...
- (IBAction)letaoGoStoreAction:(id)sender {
    if ([self.delegate respondsToSelector:@selector(letaoPushToStoreAction:)]) {
        [self.delegate letaoPushToStoreAction:self];
    }
}

- (void)letaoUpdateJDdongWithCreditLevel:(NSString *)type jd_self:(NSString *)jd_self credit_level:(NSNumber *)credit_level {
